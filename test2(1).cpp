...
void addkontainer(kontainer **pHead){
	...
	scanf("%d", &pil1);
		switch(pil1){
			case 1 :
				strcpy(newNode->jenisbarang, "makanan");
				break;
			case 2 :
				strcpy(newNode->jenisbarang, "elektronik");
				break;
			case 3 :
...
void cekkontainer(kontainer *pHead){
	...
	if (pHead == NULL){
		...
		while(pWalker->next != NULL){
			if((strcmp(pWalker->jenisbarang, "Makanan")==0) && (strcmp(pWalker->next->jenisbarang,"Kimia")==0) || (strcmp(pWalker->jenisbarang, "kimia")==0) && (strcmp(pWalker->next->jenisbarang, "makanan")==0)){
